...
def compare_values(
    ...
    *,
    ...
    equal_nan : bool, optional
        Passed to numpy.isclose. Compare NaN's as equal.
    ...
    * For scalar float-comparable types and for arbitrary-dimension, numpy.ndarray-castable, uniform-type,
    ...
    * Sets rtol to zero to match expected Psi4 behaviour, otherwise measured as:
    ...
    if numpy.iscomplexobj(expected):
        dtype = numpy.complex
    else:
        dtype = float
    ...
    digits1 = abs(int(numpy.log10(atol))) + 2
    digits_str = f"to atol={atol}"
    if rtol > 1.0e-12:
        digits_str += f", rtol={rtol}"

    isclose = numpy.isclose(cptd, xptd, rtol=rtol, atol=atol, equal_nan=equal_nan)
...
def compare(
    ...
    * For scalar exactly-comparable types and for arbitrary-dimension, numpy.ndarray-castable, uniform-type,
    ...
    isclose = numpy.asarray(xptd == cptd)
    allclose = bool(isclose.all())

    if not allclose and equal_phase:
        try:
            n_isclose = numpy.asarray(xptd == -cptd)
        except TypeError:
            pass
        else:
...
def compare_molrecs(
    ...
    *,
    ...
    if relative_geoms == "exact":
        ...
        cgeom = numpy.array(cptd["geom"]).reshape((-1, 3))
        rgeom = numpy.array(xptd["geom"]).reshape((-1, 3))
        rmsd, mill = B787(
            rgeom=rgeom,
            cgeom=cgeom,
            runiq=None,
        ...
        if cptd["fix_com"]:
            return compare(
                True,
                numpy.allclose(numpy.zeros((3)), mill.shift, atol=atol),
                "null shift",
                quiet=(verbose == 0),
        ...
        if cptd["fix_orientation"]:
            return compare(
                True,
                numpy.allclose(numpy.identity(3), mill.rotation, atol=atol),
                "null rotation",
                quiet=(verbose == 0),
                return_message=return_message,
                return_handler=return_handler,
